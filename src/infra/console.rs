...
use console::{style, measure_text_width, pad_str, Alignment};
...
impl Spinner {
    ...
    pub(crate) fn finish(self, msg: &str) {
        ...
        self.progress_bar.set_prefix(console::style("✔".to_string()).green().to_string());
    ...
    pub(crate) fn abandon(self, msg: &str) {
        let style = ProgressStyle::with_template("{prefix} {msg}").unwrap();
        self.progress_bar.set_style(style);
        self.progress_bar.set_prefix(console::style("✘".to_string()).red().to_string());
        self.progress_bar.finish_with_message(msg.to_string());
    }
}
...
impl TestProgress {
    pub(crate) fn new(len: u64) -> Self {
        let style = ProgressStyle::with_template("[{pos:>2}/{len:>2}] {msg}\n{bar:40.green}").unwrap();
    ...
    pub(crate) fn handle_test_result(&self, stdin: &str, expected: &str, output: Option<Output>, diff: bool) -> bool {
        let fail_style = ProgressStyle::with_template("[{pos:>2}/{len:>2}] {msg}\n{bar:40.red}").unwrap();
        ...
        if let Some(Output { stdout, stderr, success, duration }) = output {
            ...
            if !success {
                ...
                self.progress_bar.abandon_with_message(format!("{} Test {} {} ({:.3}s)", cross, pos, re, duration));
                if !stdout.is_empty() { report_stdout(&stdout); }
                if !stderr.is_empty() { report_stderr(&stderr); }
                return false;
            }
            if diff {
                if expected == stdout {
                    self.progress_bar.println(format!("{} Test {} {} ({:.3}s)", check, pos, ac, duration));
                    if pos == len {
                        // All tests passed
                        self.progress_bar.finish_with_message(format!("{} All sample tests passed", check));
                    } else {
                    ...
                    self.progress_bar.set_style(fail_style);
                    self.progress_bar.abandon_with_message(format!("{} Test {} {} ({:.3}s)", cross, pos, wa, duration));
                    report_diff(&expected, &stdout);
                    if !stderr.is_empty() { report_stderr(&stderr); }
                }
            } else {
                self.progress_bar.println(format!("{} Test {} {} ({:.3}s)", check, pos, ok, duration));
                self.progress_bar.suspend(|| {
                    if !stdin.is_empty() { report_stdin(&stdin); }
            ...
            self.progress_bar.set_style(fail_style);
            self.progress_bar.abandon_with_message(format!("{} Test {} {}", cross, pos, tle));
        }
        false
    }
}
...
impl SubmitProgress {
    ...
    pub(crate) fn update(&self, status_text: &str, status_class: &str) -> bool {
        static CONTINUE_CLASS: Lazy<Regex> = Lazy::new(|| Regex::new(r"result-wait|result-rejudge-wait|result-no-judge|result-compile|result-judging").unwrap());
        static CONTINUE_TEXT: Lazy<Regex> = Lazy::new(|| Regex::new(r"채점|중|Pending|Judg").unwrap());
        ...
        if CONTINUE_CLASS.is_match(status_class) || CONTINUE_TEXT.is_match(status_text) {
            ...
            if let Some(capture) = PERCENT.captures(status_text) {
                self.progress_bar.set_position(capture.get(1).unwrap().as_str().parse::<u64>().unwrap());
            }
            return false;
        }
        ...
        let result = RESULT.captures(status_class).unwrap().get(1).unwrap().as_str().to_ascii_uppercase();
        let mut color_result = console::style(if result == "RTE" { "RE".to_string() } else { result.clone() });
        if result == "AC" {
        ...
        self.progress_bar.abandon_with_message(format!("{} [{}]", status_text, color_result));
        true
    }
}
...
fn report_diff(expected: &str, output: &str) {
    ...
    let left_width = expected.lines().map(|s| measure_text_width(s) + 2).max().unwrap_or(0).max(20);
    let expected_title = style("Expected".to_string()).green().underlined().to_string();
    ...
    for op in ops {
        ...
        to_print.extend(vec![(String::new(), String::new(), String::new(), String::new()); old_range.len().max(new_range.len())]);
        left_idx = start_idx;
        right_idx = start_idx;
        for change in diff.iter_inline_changes(op) {
            let is_left = change.tag() == ChangeTag::Delete;
            if is_left {
                to_print[left_idx].0 += &style(format!("{:>4}", left_lineno)).on_green().to_string();
            }
            let is_right = change.tag() == ChangeTag::Insert;
            if is_right {
                to_print[right_idx].2 += &style(format!("{:>4}", right_lineno)).on_red().to_string();
            }
            ...
            for (highlight, s) in change.iter_strings_lossy() {
                ...
                if highlight {
                    s = if is_left { style(s).green().to_string() } else { style(s).red().to_string() };
                }
                if is_left || is_both { to_print[left_idx].1 += &s; }
                if is_right || is_both { to_print[right_idx].3 += &s; }
            }
            if is_left || is_both { left_idx += 1; left_lineno += 1; }
            if is_right || is_both { right_idx += 1; right_lineno += 1; }
        }
    }
